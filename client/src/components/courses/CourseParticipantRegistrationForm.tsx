...
import { courseParticipantFormSchema } from "@shared/schema";
import { z } from "zod";
import { Form, FormControl, FormField, FormItem, FormLabel, FormMessage } from "@/components/ui/form";
import { Loader2 } from "lucide-react";

type FormValues = z.infer<typeof courseParticipantFormSchema>;

interface CourseParticipantRegistrationFormProps {
  courseId: number;
  courseTitle: string;
  onSuccessfulRegistration?: () => void;
}

export default function CourseParticipantRegistrationForm({ 
  courseId, 
  courseTitle,
  onSuccessfulRegistration 
}: CourseParticipantRegistrationFormProps) {
  const [success, setSuccess] = useState(false);
  const { registerParticipant, isRegistering } = useCourseParticipants(courseId);

  const form = useForm<FormValues>({
    resolver: zodResolver(courseParticipantFormSchema),
    defaultValues: {
      email: "",
      courseId: courseId,
      attended: false
      ...
      <div className="w-full py-4">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-2">
            Thank you for registering for <span className="font-semibold">{courseTitle}</span>
          </p>
          <p className="text-sm text-gray-500 mb-6">
            You will receive course updates and notifications at your email address.
          </p>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
          ...
          <p className="text-xs text-center text-gray-500 pt-2">
            By registering, you'll receive course updates and information via email
          </p>
        </form>
      </Form>
    </div>
